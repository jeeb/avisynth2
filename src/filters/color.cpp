...
AVSFunction Color_filters[] = {
  { "ColorYUV",
					 ...
					 "[levels]s[opt]s[matrix]s[showyuv]b[analyze]b[autowhite]b[autogain]b[conditional]b",
           Color::Create },
  { 0 }
};
...
#define READ_CONDITIONAL(x,y) try {\
  AVSValue cv = env->GetVar("coloryuv_" x);\
  if (cv.IsFloat())\
  y = (int)(cv.AsFloat());\
} catch (IScriptEnvironment::NotFound) {}

void Color::ReadConditionals(IScriptEnvironment* env) {
