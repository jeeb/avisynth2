...
*/ // No Dynamic compilation for AMD64
}
...
/* No dynamic assmbled code for AMD64
  assemblerY.Free();
  assemblerUV.Free();
*/
}
  ...
  while (plane-->0){
    ...
    for (int yheight=0; yheight<y; yheight++) {
      ...
      for (int x=0; x<(xloops*4); x++) {
        int total = 0;
        const BYTE* srcp2 = x+srcptemp;
        for (int b=0; b<fir_filter_size; b++) {
          total += *srcp2 * cur[b];
          srcp2 += src_pitch;
        }
		total+=8192;
