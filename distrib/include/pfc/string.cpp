...
unsigned string8::replace_string(const char * s1,unsigned len1,const char * s2,unsigned len2,bool casesens,unsigned start)
{
	...
	if (len1>0)
	{
		...
		else
		{
			...
			while(ptr+len1<len)
			{
				...
				{
					count++;
					temp.add_string(s2,len2);
					ptr += len1;
				}
				else temp.add_byte(src[ptr++]);
			}
			if (ptr<len) temp.add_string(src+ptr,len-ptr);
...
unsigned string8::replace_byte(char c1,char c2,unsigned start)
{
	assert(c1>0);
	assert(c2>0);
	return replace_string(&c1,1,&c2,1,true,start);
}
...
unsigned string8::replace_char(unsigned c1,unsigned c2,unsigned start)
{
	...
	return replace_string(s1,len1,s2,len2,true,start);
}
...
void pfc_float_to_string(char * out,double val,unsigned precision,bool b_sign)
{
	...
	_i64toa((__int64)(val * pow((double)10,(int)precision)),temp,10);
